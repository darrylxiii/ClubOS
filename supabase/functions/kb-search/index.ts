...
Deno.serve(createHandler(async (req, ctx) => {
  ...
  if (!rateLimitResult.allowed) {
    ...
    return createRateLimitResponse(rateLimitResult.retryAfter || 60, ctx.corsHeaders);
  }
  ...
  const validation = validateInputSafe(kbSearchSchema, rawInput, ctx.corsHeaders);
