...
Deno.serve(createHandler(async (req, ctx) => {
  ...
  const { data: candidateProfile, error: candidateError } = await ctx.supabase
  ...
  if (candidateProfile?.user_id) {
    const { data: candidateUser } = await ctx.supabase
      .from('profiles')
      .select('full_name')
  ...
  const { data: mentionedUsers, error: usersError } = await ctx.supabase
    .from('profiles')
