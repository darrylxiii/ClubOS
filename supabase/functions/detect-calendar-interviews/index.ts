...
async function buildEmailLookupMaps(
  ...
  const tqcUserIds = userRoles?.map((r: Record<string, unknown>) => r.user_id) || [];
  ...
  if (tqcUserIds.length > 0) {
    ...
    if (tqcUsers) {
      tqcUsers.forEach((user: Record<string, unknown>) => {
        maps.tqcMap.set(String(user.email).toLowerCase(), {
  ...
  if (candidates) {
    ...
    const candidateIds = candidates.map((c: Record<string, unknown>) => c.id);
    ...
    candidates.forEach((cand: Record<string, unknown>) => {
      const app = appMap.get(cand.id);
      maps.candidateMap.set(String(cand.email).toLowerCase(), {
        candidateId: String(cand.id),
        applicationId: app?.id as string | undefined,
        jobId: app?.job_id as string | undefined,
  ...
  if (companyMembers) {
    const memberUserIds = companyMembers.map((m: Record<string, unknown>) => m.user_id);
    ...
    if (memberProfiles) {
      ...
      companyMembers.forEach((m: Record<string, unknown>) => {
        userToCompany.set(m.user_id, m.company_id);
      });

      memberProfiles.forEach((profile: Record<string, unknown>) => {
        maps.partnerMap.set(String(profile.email).toLowerCase(), {
...
Deno.serve(createHandler(async (req, ctx) => {
  ...
  for (const connection of connections) {
    ...
    try {
      const { data: eventsData, error: eventsError } = await ctx.supabase.functions.invoke(functionName, {
        body: {
          action: 'listEvents',
  ...
  allEvents.forEach(event => {
    const attendeeEmails = event.attendees?.map((a: Record<string, unknown>) => a.email).filter(Boolean) || [];
  ...
  const emailMaps = await buildEmailLookupMaps(ctx.supabase, Array.from(allAttendeeEmails));

  // Build job team assignment lookup for matching interviewers
  const jobIds = Array.from(new Set(
    allEvents
      .map(e => (e.attendees as Record<string, unknown>[] | undefined)?.map((a: Record<string, unknown>) => a.email).filter(Boolean) || [])
      .flat()
  ...
  if (jobIds.length > 0) {
    const { data: teamAssignments } = await ctx.supabase
    ...
    if (teamAssignments) {
      teamAssignments.forEach((assignment: Record<string, unknown>) => {
  ...
  const { data: existingInterviews } = await ctx.supabase
    .from('detected_interviews')
    .select('calendar_event_id, calendar_provider')
    .in('calendar_event_id', allEvents.map(e => e.id));

  const existingSet = new Set(
    (existingInterviews || []).map((ei: Record<string, unknown>) => `${ei.calendar_provider}:${ei.calendar_event_id}`)
  );
  ...
  for (const event of allEvents) {
    ...
    if (jobId && jobTeamMap.has(jobId)) {
      ...
      attendeeEmails.forEach((email: string) => {
        ...
        const teamMember = jobTeam.find((m: Record<string, unknown>) => String(m.email).toLowerCase() === normalizedEmail);
