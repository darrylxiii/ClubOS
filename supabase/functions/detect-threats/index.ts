...
Deno.serve(createHandler(async (req, ctx) => {
  ...
  const { data: bruteForceCount, error: bfError } = await ctx.supabase.rpc('detect_brute_force_attacks');
  if (bfError) {
  ...
  const { data: enumCount, error: enumError } = await ctx.supabase.rpc('detect_enumeration_attacks');
  if (enumError) {
  ...
  const { data: stuffingCount, error: stuffError } = await ctx.supabase.rpc('detect_credential_stuffing');
  if (stuffError) {
  ...
  const { data: travelCount, error: travelError } = await ctx.supabase.rpc('detect_impossible_travel');
  if (travelError) {
  ...
  const { data: suspiciousCount, error: suspiciousError } = await ctx.supabase.rpc('detect_suspicious_logins');
  if (suspiciousError) {
  ...
  const { data: rateLimitAbuse, error: rlError } = await ctx.supabase
    .from('ai_rate_limits')
  ...
  if (!rlError && rateLimitAbuse?.length > 0) {
    for (const abuse of rateLimitAbuse) {
      ...
      if (!existing) {
        await ctx.supabase.from('threat_events').insert({
  ...
  const { data: newIPs } = await ctx.supabase
    .from('login_attempts')
    .select('ip_address')
    .gte('created_at', new Date(Date.now() - 60 * 60 * 1000).toISOString())
  ...
  if (newIPs && newIPs.length > 0) {
    ...
    const { data: cachedIPs } = await ctx.supabase
      .from('ip_geo_cache')
      .select('ip_address')
    ...
    if (ipsToEnrich.length > 0) {
      ...
      ctx.supabase.functions.invoke('enrich-ip-geo', {
  ...
  ctx.supabase.functions.invoke('auto-respond-threats', {}).catch(err =>
  ...
  // Get threat summary
  const { data: summary } = await ctx.supabase.rpc('get_threat_summary');
