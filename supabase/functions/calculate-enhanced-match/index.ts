...
Deno.serve(createAuthenticatedHandler(async (req, ctx) => {
    ...
    log('info', 'Calculating match', { userId: ctx.user.id, jobId });
    ...
    if (profileError) {
      log('error', 'Error fetching profile', { profileError, userId: ctx.user.id });
    }
    ...
    if (!job) {
      ...
      return new Response(
        ...
        { headers: { ...ctx.corsHeaders, 'Content-Type': 'application/json' }, status: 404 }
      );
    }

    if (!profile) {
      log('warn', 'Profile not found', { userId: ctx.user.id });
      return new Response(
        JSON.stringify({ error: 'Profile not found', userId: ctx.user.id }),
        { headers: { ...ctx.corsHeaders, 'Content-Type': 'application/json' }, status: 404 }
      );
    }
    ...
    log('info', 'Match calculated successfully', { userId: ctx.user.id, jobId, overallScore });

    // Store match score
    const { error: upsertError } = await ctx.supabase.from('match_scores').upsert({
      user_id: ctx.user.id,
      job_id: jobId,
    ...
    if (upsertError) {
      log('error', 'Error storing match score', { upsertError, userId: ctx.user.id, jobId });
    }
