...
serve(async (req) => {
  ...
  try {
    ...
    // Intent distribution
    const intents = messages?.reduce((acc, m) => {
    ...
    const words = allContent.split(/\s+/).filter(w => w.length > 3 && !stopWords.has(w));
    ...
    console.error("Error analyzing conversation:", message);
    return new Response(
