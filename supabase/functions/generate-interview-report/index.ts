...
Deno.serve(createHandler(async (req, ctx) => {
  ...
  if (!authHeader) {
    ...
    return CommonErrors.unauthorized(ctx.corsHeaders);
  }
  ...
  console.log('[generate-interview-report] Calling Google Gemini for meeting:', meetingId);
...
Generate a comprehensive interview report with all required fields.`
  ...
  const { data: reportData, error: insertError } = await ctx.supabase
