 ...
 * Enterprise-grade: Uses Google Cloud Translation as primary, Google Gemini as fallback
...
/**
 * Call Google Gemini (Gemini) as fallback
 */
...
Deno.serve(createHandler(async (req, ctx) => {
  ...
  const googleCloudApiKey = Deno.env.get('GOOGLE_CLOUD_TRANSLATE_API_KEY');
  ...
  // Fallback to Google Gemini if Google failed or not configured
  if (translatedTexts.length === 0 && googleApiKey) {
    logger.info('Using Google Gemini as fallback provider');
    const aiResult = await translateWithAI(flatPairs, targetLanguage, googleApiKey, logger);
