...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { userContext, careerBrainContext, conversationHistory, upcomingInterviews, urgentTasks, activeApplicationsWithStages } =
      await buildUserContext(supabase, userId, conversationId);
    ...
    if (lastUserMessage && typeof lastUserMessage.content === "string") {
      if (lastUserMessage.content.startsWith("[Search:")) {
        mode = "search";
        cleanedMessages = messages.map((m: any) => m === lastUserMessage ? { ...m, content: m.content.replace(/^\[Search:\s*/, "").replace(/\]$/, "") } : m);
      } else if (lastUserMessage.content.startsWith("[Think:")) {
        mode = "think";
        cleanedMessages = messages.map((m: any) => m === lastUserMessage ? { ...m, content: m.content.replace(/^\[Think:\s*/, "").replace(/\]$/, "") } : m);
      } else if (lastUserMessage.content.startsWith("[Canvas:")) {
        mode = "canvas";
        cleanedMessages = messages.map((m: any) => m === lastUserMessage ? { ...m, content: m.content.replace(/^\[Canvas:\s*/, "").replace(/\]$/, "") } : m);
      }
    }
    ...
    const userRolesForGating = userId
      ? (await supabase.from('user_roles').select('role').eq('user_id', userId)).data?.map((r: any) => r.role) || []
      : [];
    ...
    if (clientSelectedModel === 'google/gemini-2.5-pro') selectedModel = 'google/gemini-2.5-pro';
    else if (clientSelectedModel === 'google/gemini-2.5-flash') selectedModel = 'google/gemini-2.5-flash';
    else if (clientSelectedModel === 'openai/gpt-5') selectedModel = 'openai/gpt-5';
    else if (clientSelectedModel === 'openai/gpt-5-mini') selectedModel = 'openai/gpt-5-mini';

    if (mode === "think") selectedModel = "google/gemini-2.5-pro";
...
${userContext}`;
    ...
    if (!response.ok) {
      ...
      if (response.status === 402) return new Response(JSON.stringify({ error: "Payment required, please add funds to your Lovable AI workspace." }), { status: 402, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    ...
    const savePromise = (async () => {
      ...
      try {
        ...
        if (conversationId) {
          await supabase.from("ai_conversations").update({ messages: newMessages, context: contextMeta, updated_at: new Date().toISOString() }).eq("id", conversationId);
        } else if (userId) {
          await supabase.from("ai_conversations").insert({ user_id: userId, conversation_type: "club_ai", messages: newMessages, context: { ...contextMeta, first_interaction: new Date().toISOString() } });
        }
      } catch (error) {
