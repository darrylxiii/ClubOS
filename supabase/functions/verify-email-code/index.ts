...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // IP-based rate limiting for verify attempts (10 attempts per 30 min)
    const ipRateLimit = await checkIPRateLimit(ipAddress, email, 'email', 10, 30 * 60 * 1000);
    ...
    if (user) {
      ...
      // Fallback to plaintext for codes created before hash migration
      if (!result.data) {
