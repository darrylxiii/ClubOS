...
export const handleGenerateMLFeatures = async ({ supabase, payload }: { supabase: any; payload: any }) => {
    const { candidate_id, job_id, application_id, use_cache = true } = payload as FeatureRequest;
    ...
    const features: Record<string, any> = {
        ...
        // Application History
        candidate_applied_to_company_before: candidateApplications.some((app: any) =>
            jobApplications.some((jobApp: any) => jobApp.id === app.id)
        ) ? 1 : 0,
