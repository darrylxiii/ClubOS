...
export async function compileTranscript(reqBody: any, supabaseClient?: any) {
    const { meetingId, meeting_id, recordingId, recording_id } = reqBody;
    ...
    const transcriptJson = transcripts.map((t: any) => ({
    ...
    const plainTextTranscript = transcripts.map((t: any) => {
...
export const compileTranscriptHandler = async (req: Request) => {
    ...
    try {
        ...
        const result = await compileTranscript(body);
