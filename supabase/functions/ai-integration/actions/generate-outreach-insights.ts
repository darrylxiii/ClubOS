...
export async function handleGenerateOutreachInsights({ supabase, payload }: ActionContext) {
    ...
    const totalOpened = campaigns?.reduce((sum: number, c: any) => sum + (c.total_opened || 0), 0) || 0;
    ...
    const avgHealthScore = accounts?.length
        ? accounts.reduce((sum: number, a: any) => sum + (a.health_score || 0), 0) / accounts.length
        : 100;

    const highProbabilityLeads = predictions?.filter((p: any) => p.conversion_probability >= 70).length || 0;
    ...
    if (highProbabilityLeads > 0) {
        insights.push({
            ...
            recommendations: predictions?.slice(0, 3).map((p: any) => p.recommended_action) || [],
            severity: 'positive',
        });
    }
    ...
    return { success: true, executiveSummary, insights };
}
