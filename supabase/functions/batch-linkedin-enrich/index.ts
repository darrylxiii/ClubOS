...
Deno.serve(createAuthenticatedHandler(async (req, ctx) => {
  ...
  console.log(`[batch-linkedin-enrich] Processing ${candidate_ids.length} candidates for user ${ctx.user.id}`);

  // Fetch candidate profiles
  const { data: candidates, error: fetchError } = await ctx.supabase
