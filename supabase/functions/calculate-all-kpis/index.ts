...
Deno.serve(createHandler(async (req, ctx) => {
  ...
  const { count: tasksCompleted } = await ctx.supabase
    .from('unified_tasks')
    .select('*', { count: 'exact', head: true })
    .eq('status', 'done')
    .gte('completed_at', sevenDaysAgo.toISOString());

  const { count: tasksTotal } = await ctx.supabase
    .from('unified_tasks')
    .select('*', { count: 'exact', head: true })
  ...
  // Meetings
  const { count: candidateMeetings } = await ctx.supabase
    .from('meetings')
    .select('*', { count: 'exact', head: true })
    .eq('meeting_type', 'interview')
    .gte('scheduled_start', sevenDaysAgo.toISOString());

  const { count: clientMeetings } = await ctx.supabase
    .from('meetings')
    .select('*', { count: 'exact', head: true })
  ...
  const { count: referralApps } = await ctx.supabase
    .from('applications')
    .select('*', { count: 'exact', head: true })
  ...
  const { data: pageAnalytics } = await ctx.supabase
  ...
  const { data: sessionEvents } = await ctx.supabase
  ...
  const { count: bookingsTotal } = await ctx.supabase
    .from('bookings')
    .select('*', { count: 'exact', head: true })
    .gte('scheduled_start', sevenDaysAgo.toISOString());

  const { count: bookingsAttended } = await ctx.supabase
    .from('bookings')
    .select('*', { count: 'exact', head: true })
    .gte('scheduled_start', sevenDaysAgo.toISOString())
    .eq('status', 'completed');
  ...
  for (const kpi of results.website) {
    ...
    await ctx.supabase.from('web_kpi_metrics').insert({
      category: kpi.category,
      kpi_name: kpi.kpi_name,
      value: kpi.value,
  ...
  for (const kpi of results.sales) {
    ...
    await ctx.supabase.from('sales_kpi_metrics').insert({
      category: kpi.category,
      kpi_name: kpi.kpi_name,
      value: kpi.value,
      target_value: kpi.target_value,
  ...
  await ctx.supabase.rpc('calculate_kpi_trends');
