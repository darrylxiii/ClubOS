...
Deno.serve(createHandler(async (req, ctx) => {
  ...
  const { data: marker, error: markerError } = await ctx.supabase
  ...
  const { data: verifyMarker, error: verifyError } = await ctx.supabase
    .from('pitr_test_markers')
    .select('*')
    .eq('test_id', testId)
    .single();
  ...
  if (!testSuccess) {
    const { error: alertError } = await ctx.supabase
  ...
  const { data: oldMarkers } = await ctx.supabase
    .from('pitr_test_markers')
    .select('id')
