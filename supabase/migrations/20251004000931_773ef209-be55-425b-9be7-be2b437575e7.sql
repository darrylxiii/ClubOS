-- ============================================
-- ENHANCED COMPANY PROFILE MODULE (Updated)
-- Posts, Analytics, Branding, Followers
-- ============================================

-- Drop existing index if it exists
DROP INDEX IF EXISTS public.idx_company_posts_company;
DROP INDEX IF EXISTS public.idx_company_posts_published;
DROP INDEX IF EXISTS public.idx_company_posts_featured;
...
CREATE INDEX IF NOT EXISTS idx_company_posts_company ON public.company_posts(company_id);
CREATE INDEX IF NOT EXISTS idx_company_posts_published ON public.company_posts(published_at) WHERE published_at IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_company_posts_featured ON public.company_posts(is_featured) WHERE is_featured = true;
...
CREATE INDEX IF NOT EXISTS idx_post_reactions_post ON public.company_post_reactions(post_id);
CREATE INDEX IF NOT EXISTS idx_post_reactions_user ON public.company_post_reactions(user_id);
...
CREATE INDEX IF NOT EXISTS idx_post_comments_post ON public.company_post_comments(post_id);
...
CREATE INDEX IF NOT EXISTS idx_company_followers_user ON public.company_followers(user_id);
...
CREATE INDEX IF NOT EXISTS idx_company_analytics_company ON public.company_analytics(company_id);
CREATE INDEX IF NOT EXISTS idx_company_analytics_date ON public.company_analytics(date DESC);
...
CREATE INDEX IF NOT EXISTS idx_job_analytics_job ON public.job_analytics(job_id);
CREATE INDEX IF NOT EXISTS idx_job_analytics_date ON public.job_analytics(date DESC);
...
DROP POLICY IF EXISTS "Public posts are viewable by everyone" ON public.company_posts;
CREATE POLICY "Public posts are viewable by everyone"
  ON public.company_posts FOR SELECT
  USING (is_public = true AND published_at IS NOT NULL);

DROP POLICY IF EXISTS "Company members can view their company posts" ON public.company_posts;
CREATE POLICY "Company members can view their company posts"
  ON public.company_posts FOR SELECT
  USING (is_company_member(auth.uid(), company_id));

DROP POLICY IF EXISTS "Company members can create posts" ON public.company_posts;
...
DROP POLICY IF EXISTS "Anyone can view reactions" ON public.company_post_reactions;
CREATE POLICY "Anyone can view reactions"
  ON public.company_post_reactions FOR SELECT
  USING (true);

DROP POLICY IF EXISTS "Users can create their own reactions" ON public.company_post_reactions;
CREATE POLICY "Users can create their own reactions"
  ON public.company_post_reactions FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete their own reactions" ON public.company_post_reactions;
...
DROP POLICY IF EXISTS "Anyone can view comments on public posts" ON public.company_post_comments;
...
DROP POLICY IF EXISTS "Users can create comments" ON public.company_post_comments;
CREATE POLICY "Users can create comments"
  ON public.company_post_comments FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own comments" ON public.company_post_comments;
CREATE POLICY "Users can update their own comments"
  ON public.company_post_comments FOR UPDATE
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete their own comments" ON public.company_post_comments;
...
DROP POLICY IF EXISTS "Users can view their own follows" ON public.company_followers;
CREATE POLICY "Users can view their own follows"
  ON public.company_followers FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Company members can view followers" ON public.company_followers;
...
DROP POLICY IF EXISTS "Company admins can manage branding" ON public.company_branding;
...
DROP POLICY IF EXISTS "Company members can view analytics" ON public.company_analytics;
...
DROP POLICY IF EXISTS "Company members can view job analytics" ON public.job_analytics;
...
DROP TRIGGER IF EXISTS update_company_post_comments_updated_at ON public.company_post_comments;
