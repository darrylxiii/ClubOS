...
-- This addresses Supabase linter warning: SUPA_function_search_path_mutable
...
-- 2. Fix calculate_post_score()
CREATE OR REPLACE FUNCTION public.calculate_post_score(p_user_id uuid, p_post_id uuid, p_post_created_at timestamp with time zone, p_post_author_id uuid, p_likes_count integer, p_comments_count integer, p_shares_count integer)
RETURNS numeric
LANGUAGE plpgsql
...
-- 3. Fix update_relationship_score()
CREATE OR REPLACE FUNCTION public.update_relationship_score(p_user_id uuid, p_related_user_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = 'public'
...
-- 4. Fix update_achievement_analytics()
...
-- 6. Fix evaluate_user_achievements()
