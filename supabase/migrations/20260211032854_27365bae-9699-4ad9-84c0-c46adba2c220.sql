...
BEGIN
  ...
  -- desired_locations is jsonb, check if array with items
  IF v_candidate.desired_locations IS NOT NULL AND jsonb_array_length(v_candidate.desired_locations) > 0 THEN v_score := v_score + 3; END IF;
