    ...
    <div className="w-full px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <Tabs key={activeTab} value={activeTab} onValueChange={setActiveTabValue}>
        <TabsList className="w-full justify-start flex-wrap h-auto gap-1 py-1">
          ...
          <TabsTrigger value="my-meetings" className="gap-2"><Video className="h-4 w-4" />My Meetings</TabsTrigger>
          <TabsTrigger value="prep" className="gap-2"><FileText className="h-4 w-4" />Prep</TabsTrigger>
          <TabsTrigger value="post" className="gap-2"><CheckSquare className="h-4 w-4" />Post</TabsTrigger>
        ...
        <TabsContent value="my-meetings" className="mt-6">
          <div className="space-y-4">
            <div className="relative">
              ...
              <Input placeholder="Search meetings..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10" />
            </div>
            {loading ? (
              <div className="space-y-4">
        ...
        <TabsContent value="prep" className="mt-6">
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Generate dossiers and prepare for upcoming meetings</p>
            </div>
            {loading ? (
              <div className="space-y-4">
                  ...
                  .map((meeting) => (
                    <Card key={meeting.id} className="p-4">
                      <div className="flex items-start justify-between">
                        ...
                        <Button variant="outline" size="sm" onClick={() => navigate(`/meeting-notes/${meeting.id}`)}>Prepare</Button>
                      </div>
                    </Card>
                  ))}
              </div>
        ...
        <TabsContent value="post" className="mt-6">
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">View summaries, action items, and follow-ups for completed meetings</p>
            </div>
            {loading ? (
              <div className="space-y-4">
                {[1, 2, 3].map((i) => <Skeleton key={i} className="h-24 w-full" />)}
                  ...
                  .map((meeting) => (
                    <Card key={meeting.id} className="p-4">
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex gap-2">
                          <Button variant="outline" size="sm" onClick={() => navigate(`/meeting-notes/${meeting.id}`)}>View Notes</Button>
