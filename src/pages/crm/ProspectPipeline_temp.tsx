...
function ProspectPipelineContent() {
    ...
    if (owners.length > 0) {
      const assignToOwner = async () => {
        ...
        try {
          ...
          toast.error(t('toast.failedToAssignProspects', 'Failed to assign prospects'));
        }
      };
      assignToOwner();
    } else {
      toast.info(t('toast.noOwnersAvailableForAssignment', 'No owners available for assignment'));
    }
  ...
  const handleSyncFromInstantly = async () => {
    ...
    toast.info(t('toast.startingSyncFromInstantly', 'Starting sync from Instantly…'));
    ...
    try {
      ...
      if (imported > 0 || updated > 0) {
        ...
        toast.info(t('toast.allLeadsAreUpTo', 'All leads are up to date'));
      }
    } catch (error: unknown) {
      clearTimeout(timeoutId);
      ...
      if (err.name === 'AbortError' || controller.signal.aborted) {
        toast.info(t('toast.syncIsStillRunningIn', 'Sync is still running in the background. Results will appear after refresh.'));
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)]">
      ...
      <motion.div
        ...
        className="flex-shrink-0 px-6 py-4 border-b border-border/30 bg-gradient-to-r from-card/50 to-transparent backdrop-blur-[var(--blur-glass)]"
      >
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="flex items-center gap-3">
            <div>
              <h1 className="text-2xl font-bold">{t('prospectPipeline', 'Prospect Pipeline')}</h1>
          ...
          <div className="flex flex-wrap items-center gap-2">
              ...
              <PopoverContent className="w-80 p-4" align="end">
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h4 className="font-medium">{t('advancedFilters', 'Advanced Filters')}</h4>
                    {hasActiveFilters && (
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <Label>{t('leadScoreRange', 'Lead Score Range')}</Label>
                  ...
                  >{t('applyFilters', 'Apply Filters')}</Button>
                </div>
              </PopoverContent>
            </Popover>
              ...
              <TooltipTrigger asChild>
                ...
                >
                  {syncingInstantly ? (
                    ...
                    <>
                      <Zap className="w-4 h-4 mr-2 text-purple-500" />{t('syncInstantly', 'Sync Instantly')}</>
                  )}
                </Button>
              </TooltipTrigger>
              <TooltipContent>
            ...
            <Button variant="outline" size="icon" onClick={() => refetch()} aria-label={t('aria-label.refreshProspects', 'Refresh prospects')}>
            ...
            <Button onClick={() => handleAddProspect('new')}>
              <Plus className="w-4 h-4 mr-2" />{t('addProspect', 'Add Prospect')}</Button>

            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon" onClick={() => setShowHelp(true)} aria-label={t('aria-label.keyboardShortcuts', 'Keyboard shortcuts')}>
