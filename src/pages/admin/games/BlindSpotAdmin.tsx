...
import { Eye, Users, Clock, Search, ArrowLeft, Brain } from "lucide-react";
...
export default function BlindSpotAdmin() {
  const [searchQuery, setSearchQuery] = useState("");
  const navigate = useNavigate();

  const { data: results, isLoading } = useQuery({
    queryKey: ["admin-blind-spot-results"],
    queryFn: async () => {
  ...
  const { data: stats } = useQuery({
    queryKey: ["admin-blind-spot-stats"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("assessment_results")
      ...
      <RoleGate allowedRoles={["admin", "strategist"]}>
        <div className="container mx-auto px-4 py-6 max-w-7xl space-y-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <PageTitle>Blind Spot Detector Admin</PageTitle>
              <p className="text-muted-foreground">
                Manage and view all Blind Spot Detector assessment results
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Card className="bg-gradient-to-br from-card/90 to-card/60 backdrop-blur-xl">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <CardTitle className="text-sm font-medium">Avg. Self-Awareness</CardTitle>
