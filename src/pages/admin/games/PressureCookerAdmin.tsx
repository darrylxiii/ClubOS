...
import { Flame, Users, Clock, Search, ArrowLeft, Eye, Zap } from "lucide-react";
...
export default function PressureCookerAdmin() {
  ...
  const { data: results, isLoading } = useQuery({
    queryKey: ["admin-pressure-cooker-results"],
    queryFn: async () => {
  ...
  const { data: stats } = useQuery({
    queryKey: ["admin-pressure-cooker-stats"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("assessment_results")
      ...
      <RoleGate allowedRoles={["admin", "strategist"]}>
        <div className="container mx-auto px-4 py-6 max-w-7xl space-y-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <PageTitle>Pressure Cooker Admin</PageTitle>
              <p className="text-muted-foreground">
                Manage and view all Pressure Cooker assessment results
              </p>
            </div>
          </div>
