...
import { Trophy, Users, Clock, Search, ArrowLeft, Eye } from "lucide-react";
...
export default function ValuesPokerAdmin() {
  const [searchQuery, setSearchQuery] = useState("");
  const navigate = useNavigate();

  const { data: results, isLoading } = useQuery({
    queryKey: ["admin-values-poker-results"],
    queryFn: async () => {
  ...
  const { data: stats } = useQuery({
    queryKey: ["admin-values-poker-stats"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("assessment_results")
      ...
      <RoleGate allowedRoles={["admin", "strategist"]}>
        <div className="container mx-auto px-4 py-6 max-w-7xl space-y-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <PageTitle>Values Poker Admin</PageTitle>
              <p className="text-muted-foreground">
                Manage and view all Values Poker assessment results
              </p>
            </div>
          </div>
