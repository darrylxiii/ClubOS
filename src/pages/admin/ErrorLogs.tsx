...
import { AlertTriangle, Search, RefreshCw, CheckCircle, XCircle, AlertCircle, Info, Clock } from 'lucide-react';
...
export default function ErrorLogs() {
  ...
  const { data: errorLogs, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase.from('error_analytics_summary').select('*').order('created_at', { ascending: false }).limit(100);
  ...
  const { data: errorStats } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('error_analytics_summary').select('severity').limit(1000);
  ...
  const filteredLogs = errorLogs?.filter(log => {
    ...
    return log.error_type?.toLowerCase().includes(query) || log.message?.toLowerCase().includes(query) || log.page_url?.toLowerCase().includes(query);
  });
            ...
            <ScrollArea className="h-[500px]">
              <div className="space-y-3">
                {filteredLogs.map((log: any) => {
                  ...
                  return (
                    <div key={log.id} className={`p-4 border rounded-lg ${config.bg}`}>
                      <div className="flex items-start gap-3">
                        <Icon className={`h-5 w-5 mt-0.5 ${config.color}`} />
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <Badge variant="outline" className={config.color}>{log.severity?.toUpperCase()}</Badge>
                            <span className="text-sm font-medium truncate">{log.error_type || 'Unknown Error'}</span>
                          </div>
                          <p className="text-sm text-muted-foreground mb-2">{log.message || 'No message provided'}</p>
                          <div className="flex items-center gap-4 text-xs text-muted-foreground">
                            <span className="flex items-center gap-1"><Clock className="h-3 w-3" />{log.created_at ? format(new Date(log.created_at), 'MMM d, HH:mm:ss') : 'Unknown time'}</span>
                            {log.page_url && <span className="truncate max-w-[200px]">{log.page_url}</span>}
                          </div>
                        </div>
                        <Badge variant={log.resolved ? 'secondary' : 'destructive'}>{log.resolved ? 'Resolved' : 'Open'}</Badge>
                      </div>
                    </div>
                  );
                })}
