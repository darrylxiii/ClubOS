...
export default function BrandTermManager() {
  ...
  const handleSave = async () => {
    if (!formData.term.trim()) { toast.error(t('brandTerms.termRequired')); return; }
    try {
      if (editingTerm) { await updateTerm.mutateAsync({ id: editingTerm.id, ...formData }); toast.success(t('brandTerms.termUpdated')); }
      else { await createTerm.mutateAsync(formData); toast.success(t('brandTerms.termCreated')); }
      setIsDialogOpen(false);
    } catch { toast.error(t('brandTerms.saveFailed')); }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-end">
        <Button onClick={handleOpenCreate}><Plus className="h-4 w-4 mr-2" />{t('brandTerms.addTerm')}</Button>
      </div>
      
      <Card>
        <CardHeader>
          <CardTitle>{t('brandTerms.protectedTerms')}</CardTitle>
          <CardDescription>{t('brandTerms.protectedTermsDesc')}</CardDescription>
        </CardHeader>
        <CardContent>
