...
export default function BrandTermManager() {
  ...
  const handleOpenEdit = (term: BrandTerm) => {
    setEditingTerm(term);
    setFormData({
      term: term.term,
      description: term.description || '',
      never_translate: term.never_translate,
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingTerm) {
        ...
        toast.success('Brand term updated');
      } else {
        await createTerm.mutateAsync(formData);
        toast.success('Brand term created');
      }
      setIsDialogOpen(false);
    } catch (error) {
      toast.error('Failed to save brand term');
    }
  };
        ...
        <div className="container mx-auto px-4 py-6 max-w-6xl">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-muted-foreground">
                Protect brand terms from AI translation or provide custom translations
              </p>
            </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Terms marked as "never translate" will be preserved as-is in all languages. 
                Terms with custom translations will use those instead of AI-generated ones.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {isLoading ? (
                  ...
                  <TableBody>
                    {brandTerms?.map((term) => (
                      <TableRow key={term.id}>
                        ...
                        <TableCell>
                          <Badge variant="outline">{term.priority}</Badge>
                        </TableCell>
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>
                  {editingTerm ? 'Edit Brand Term' : 'Add Brand Term'}
                </DialogTitle>
              </DialogHeader>
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="e.g., The Quantum Club"
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    placeholder="Why this term is protected"
                  />
                </div>
                
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    <Label>Never Translate</Label>
                    <p className="text-sm text-muted-foreground">
                      Keep this term exactly as-is in all languages
                    </p>
                  </div>
                  <Switch
                    checked={formData.never_translate}
                    onCheckedChange={(checked) => setFormData({ ...formData, never_translate: checked })}
                  />
                </div>
                ...
                {!formData.never_translate && (
                  <div className="space-y-3">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Provide specific translations for each language
                    </p>
