  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-end">
        <Button onClick={handleRefresh} variant="outline" className="gap-2"><RefreshCw className={`h-4 w-4 ${isLoading ? 'animate-spin' : ''}`} />Refresh</Button>
      </div>
              ...
              <div className="space-y-4">
                {Object.entries(coverage?.byLanguage || {}).map(([lang, data]) => (
                  <div key={lang} className="flex items-center gap-4 p-4 rounded-lg bg-muted/50">
                    <div className="w-12 text-center"><span className="text-2xl">{getLanguageFlag(lang)}</span><p className="text-xs font-medium uppercase mt-1">{lang}</p></div>
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-2"><span className="font-medium">{getLanguageName(lang)}</span><div className="flex items-center gap-2">{getQualityBadge((data as any).qualityScore)}{getStatusBadge((data as any).percentage || 0)}</div></div>
                      <Progress value={(data as any).percentage || 0} className="h-2" />
                      <div className="flex justify-between text-xs text-muted-foreground mt-1"><span>{(data as any).translated || 0} / {(data as any).total || 0} keys</span><span className={getStatusColor((data as any).percentage || 0)}>{((data as any).percentage || 0).toFixed(1)}%</span></div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {Object.entries(coverage?.byNamespace || {}).map(([ns, data]) => (
                  <div key={ns} className="p-4 rounded-lg bg-muted/50 border">
                    <div className="flex items-center justify-between mb-3"><span className="font-medium capitalize">{ns}</span>{getStatusBadge((data as any).percentage || 0)}</div>
                    <Progress value={(data as any).percentage || 0} className="h-2 mb-2" />
                    <p className="text-xs text-muted-foreground">{(data as any).translated || 0} / {(data as any).total || 0} translations complete</p>
                  </div>
                ))}
              </div>
            </CardContent>
                ...
                <div className="space-y-3">
                  {(needsReviewList || []).map(item => (
                    <div key={item.id} className="flex items-center justify-between p-4 rounded-lg bg-muted/50 border border-yellow-500/30">
                      <div className="flex items-center gap-4"><span className="text-2xl">{getLanguageFlag(item.language)}</span><div><p className="font-medium">{item.namespace}</p><div className="flex items-center gap-2 mt-1"><Badge variant="outline" className="text-xs">{item.language.toUpperCase()}</Badge>{item.translation_provider && <Badge variant="secondary" className="text-xs">{item.translation_provider === 'google' ? '🌐 Google' : '🤖 AI'}</Badge>}{item.quality_score && <Badge className="bg-yellow-500/20 text-yellow-600 text-xs">Score: {item.quality_score}</Badge>}</div></div></div>
                      <Button size="sm" variant="outline" onClick={() => handleMarkReviewed(item.id)} disabled={markAsReviewed.isPending}><CheckCircle2 className="h-4 w-4 mr-1" />Mark Reviewed</Button>
                    </div>
                  ))}
                </div>
              )}
              ...
              {(coverage?.missingKeys || []).length === 0 ? (
                ...
                <div className="space-y-2 max-h-96 overflow-y-auto">
                  {(coverage?.missingKeys || []).slice(0, 50).map((item: any, index: number) => (
                    <div key={index} className="flex items-center justify-between p-3 rounded-lg bg-muted/50 text-sm"><div><code className="text-xs bg-background px-2 py-1 rounded">{item.namespace}:{item.key}</code></div><div className="flex gap-1">{item.missingIn?.map((lang: string) => <Badge key={lang} variant="outline" className="text-xs">{lang}</Badge>)}</div></div>
                  ))}
                  {(coverage?.missingKeys || []).length > 50 && <p className="text-center text-muted-foreground text-sm py-4">And {(coverage?.missingKeys || []).length - 50} more...</p>}
                </div>
              )}
            </CardContent>
          </Card>
...
function getLanguageFlag(code: string): string {
  const flags: Record<string, string> = { en: '🇬🇧', nl: '🇳🇱', de: '🇩🇪', fr: '🇫🇷', es: '🇪🇸', zh: '🇨🇳', ar: '🇸🇦', ru: '🇷🇺' };
  return flags[code] || '🌐';
}

function getLanguageName(code: string): string {
  const names: Record<string, string> = { en: 'English', nl: 'Dutch', de: 'German', fr: 'French', es: 'Spanish', zh: 'Chinese', ar: 'Arabic', ru: 'Russian' };
