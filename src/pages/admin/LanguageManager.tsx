...
import { Plus, Globe, Check, Loader2, Languages, RefreshCw, Star, AlertTriangle, Bot } from "lucide-react";
...
export default function LanguageManager() {
  ...
  const { data: languages, isLoading } = useQuery({
    queryKey: ['languages'],
    queryFn: async () => { const { data, error } = await supabase.from('language_config').select('*').order('code'); if (error) throw error; return data; },
  });

  const { data: dbNamespaces } = useQuery({
    queryKey: ['db-namespaces'],
    queryFn: async () => { const { data, error } = await supabase.from('translations').select('namespace').eq('language', 'en').eq('is_active', true); if (error) throw error; return [...new Set(data.map(t => t.namespace))]; },
  });

  const { data: coverage, refetch: refetchCoverage } = useQuery({
    queryKey: ['translation-coverage-detailed'],
    queryFn: async () => {
      const { data, error } = await supabase.from('translations').select('namespace, language, quality_score, quality_status, translation_provider').eq('is_active', true);
      if (error) throw error;
      const coverageMap: Record<string, { namespaces: Set<string>; qualitySum: number; qualityCount: number; needsReview: number; provider: string | null }> = {};
      data.forEach(t => {
        if (!coverageMap[t.language]) coverageMap[t.language] = { namespaces: new Set(), qualitySum: 0, qualityCount: 0, needsReview: 0, provider: null };
        coverageMap[t.language].namespaces.add(t.namespace);
        if (t.quality_score !== null) { coverageMap[t.language].qualitySum += t.quality_score; coverageMap[t.language].qualityCount++; }
        if (t.quality_status === 'needs_review') coverageMap[t.language].needsReview++;
        if (t.translation_provider && !coverageMap[t.language].provider) coverageMap[t.language].provider = t.translation_provider;
      });
      return coverageMap;
    },
  });
  ...
  const addLanguageMutation = useMutation({
    mutationFn: async (language: typeof newLanguage & { autoGenerate: boolean }) => {
      const { error: configError } = await supabase.from('language_config').insert({ code: language.code, name: language.name, native_name: language.nativeName, flag_emoji: language.flagEmoji, is_rtl: language.isRtl, font_family: language.fontFamily || null });
      if (configError) throw configError;
      if (language.autoGenerate) { setIsGenerating(true); const { data, error: genError } = await supabase.functions.invoke('generate-all-translations', { body: { generateAll: true } }); if (genError) throw new Error('Language added but translation generation failed'); return data; }
      return { success: true };
    },
    onSuccess: () => {
  ...
  const handleQuickAdd = async (lang: typeof popularLanguages[0]) => {
    await addLanguageMutation.mutateAsync({ code: lang.code, name: lang.name, nativeName: lang.native, flagEmoji: lang.flag, isRtl: false, fontFamily: '', autoGenerate: true });
  };
  ...
  const getCoverage = (langCode: string) => {
    ...
    const avgQuality = langData.qualityCount > 0 ? Math.round(langData.qualitySum / langData.qualityCount) : null;
    return { count: langData.namespaces.size, total: totalNamespaces, percentage: Math.round((langData.namespaces.size / totalNamespaces) * 100), quality: avgQuality, needsReview: langData.needsReview, provider: langData.provider };
  };

  const isComplete = (langCode: string) => { const { count, total } = getCoverage(langCode); return count >= total; };
  const getQualityColor = (quality: number | null) => quality === null ? 'text-muted-foreground' : quality >= 90 ? 'text-green-500' : quality >= 70 ? 'text-yellow-500' : 'text-orange-500';
          ...
          <CardContent>
            <div className="flex flex-wrap gap-2">
              {popularLanguages.map(lang => {
                ...
                return <Button key={lang.code} variant="outline" size="sm" onClick={() => handleQuickAdd(lang)} disabled={alreadyExists || isGenerating}>{lang.flag} {lang.name}{alreadyExists && <Check className="ml-2 h-3 w-3 text-green-500" />}</Button>;
              })}
            </div>
          </CardContent>
        </Card>
