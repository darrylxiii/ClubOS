...
import { useNamespaceTranslations, flattenTranslations, useUpdateTranslation, useMarkAsReviewed, type TranslationRecord } from '@/hooks/use-translation-editor';
...
export default function TranslationEditor() {
  ...
  const flatTranslations = useMemo(() => translations ? flattenTranslations(translations) : [], [translations]);
  const filteredTranslations = useMemo(() => {
    if (!searchQuery) return flatTranslations;
    const query = searchQuery.toLowerCase();
    return flatTranslations.filter(t => t.key.toLowerCase().includes(query) || Object.values(t.values).some(v => v.toLowerCase().includes(query)));
  }, [flatTranslations, searchQuery]);

  const getTranslationRecord = (lang: string): TranslationRecord | undefined => translations?.find(t => t.language === lang);
  ...
  const handleExport = () => {
    ...
    const exportData = translations.reduce((acc, t) => { acc[t.language] = t.translations; return acc; }, {} as Record<string, any>);
  ...
  const unreviewedCount = translations?.filter(t => t.language !== 'en' && t.quality_status !== 'validated').length || 0;
                  ...
                  <tbody>
                    {filteredTranslations.map(translation => (
                      <tr key={translation.key} className="border-b hover:bg-muted/30">
                        <td className="p-3 font-mono text-sm text-muted-foreground">{translation.key}</td>
                        {activeLanguages.map(lang => {
                          ...
                          return (
                            <td key={lang.code} className={`p-3 ${hasWarning ? 'bg-yellow-500/10' : ''}`}>
                              {isEditing ? (
                                ...
                                <div className="group flex items-start gap-2 cursor-pointer hover:bg-muted/50 p-2 rounded" onClick={() => handleStartEdit(translation.key, lang.code, value)}>
                                  <span className={`flex-1 text-sm ${!value ? 'text-muted-foreground italic' : ''}`}>{value || '(empty)'}</span>
