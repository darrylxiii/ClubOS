    ...
    switch (errorType) {
      case 'size':
        return {
          ...
          description: t("errors.fileTooLargeDesc", "The file you're trying to upload exceeds the size limit."),
          suggestion: t("errors.fileTooLargeSuggestion", "Please compress the file or choose a smaller one. Maximum size is typically 10MB."),
        };
      case 'type':
        return {
          icon: <FileX className="h-8 w-8 text-amber-500" />,
          title: t("errors.unsupportedFileType", "Unsupported File Type"),
          description: t("errors.unsupportedFileTypeDesc", "This file format is not supported."),
          suggestion: t("errors.unsupportedFileTypeSuggestion", "Please upload a file in a supported format (PDF, DOCX, PNG, JPG, etc.)."),
        };
      case 'network':
        return {
          icon: <AlertTriangle className="h-8 w-8 text-amber-500" />,
          title: t("errors.uploadInterrupted", "Upload Interrupted"),
          description: t("errors.uploadInterruptedDesc", "The upload was interrupted due to a network issue."),
          suggestion: t("errors.uploadInterruptedSuggestion", "Check your internet connection and try again. The file will not be partially uploaded."),
        };
      case 'storage':
        return {
          icon: <AlertTriangle className="h-8 w-8 text-destructive" />,
          title: t("errors.storageError", "Storage Error"),
          description: t("errors.storageErrorDesc", "There was a problem with the file storage service."),
          suggestion: t("errors.storageErrorSuggestion", "Please try again later. If the problem persists, contact support."),
        };
      default:
        return {
          icon: <Upload className="h-8 w-8 text-destructive" />,
          title: t("errors.uploadFailed", "Upload Failed"),
          description: error?.message || t("errors.unexpectedUploadError", "An unexpected error occurred during upload."),
          suggestion: t("errors.uploadRetrySuggestion", "Please try uploading the file again."),
        };
    }
  };
