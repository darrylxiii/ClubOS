  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>{t("submit_timesheet", "Submit Timesheet")}</DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4 p-4 rounded-lg bg-muted/30">
            <div className="text-center">
              ...
              <p className="text-xs text-muted-foreground">{t("total_hours", "Total Hours")}</p>
            </div>
            <div className="text-center">
          ...
          <div className="space-y-3">
            <Label>{t("presubmission_checks", "Pre-Submission Checks")}</Label>
                ...
                {validation.errors.length > 0 && (
                  <div className="p-3 rounded-lg bg-destructive/10 border border-destructive/20">
                    <div className="flex items-center gap-2 text-destructive mb-2">
                      ...
                      <span className="font-medium">{t("errors", "Errors")}</span>
                    </div>
                ...
                {validation.warnings.length > 0 && (
                  <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
                    <div className="flex items-center gap-2 text-amber-400 mb-2">
                      ...
                      <span className="font-medium">{t("warnings", "Warnings")}</span>
                    </div>
                ...
                {validation.errors.length === 0 && validation.warnings.length === 0 && (
                  <div className="p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
                    <div className="flex items-center gap-2 text-emerald-400">
                      ...
                      <span className="font-medium">{t("all_checks_passed", "All checks passed")}</span>
                    </div>
                  </div>
                )}
              </div>
          ...
          {/* Notes */}
          <div className="space-y-2">
            <Label htmlFor="notes">{t("notes_for_approver_optional", "Notes for Approver (Optional)")}</Label>
            <Textarea
              id="notes"
              placeholder={t("add_any_notes_or", "Add any notes or context for your approver...")}
              value={notes}
