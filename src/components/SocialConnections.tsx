...
export function SocialConnections({
  ...
  const handleConnectSpotify = async () => {
    ...
    try {
      ...
      toast.success(t("spotify_connected", "Spotify connected!"));
      onUpdate();
    } catch (error) {
      console.error('Error connecting Spotify:', error);
      toast.error(t("failed_to_connect_spotify", "Failed to connect Spotify"));
    }
  };
  ...
  const handleConnectAppleMusic = async () => {
    ...
    try {
      ...
      toast.success(t("apple_music_connected", "Apple Music connected!"));
      onUpdate();
    } catch (error) {
      console.error('Error connecting Apple Music:', error);
      toast.error(t("failed_to_connect_apple", "Failed to connect Apple Music"));
    }
  };
  ...
  const handleAddSpotifyPlaylist = async () => {
    ...
    const newPlaylist: Playlist = {
      id: playlistId,
      name: spotifyType === 'playlist' ? t('socialconnections.spotifyPlaylist', 'Spotify Playlist') : t('socialconnections.spotifyPodcast', 'Spotify Podcast'),
      url: spotifyUrl,
      type: spotifyType,
    };
    ...
    try {
      ...
      toast.success(`${spotifyType === 'playlist' ? t('socialconnections.playlist', 'Playlist') : t('socialconnections.podcast', 'Podcast')} added!`);
      setSpotifyUrl('');
      setShowSpotifyInput(false);
      onUpdate();
    } catch (error) {
      console.error('Error adding playlist:', error);
      toast.error(t("failed_to_add_playlist", "Failed to add playlist"));
    }
  };
  ...
  const handleAddAppleMusicPlaylist = async () => {
    ...
    const newPlaylist: Playlist = {
      id: playlistId,
      name: appleMusicType === 'playlist' ? t('socialconnections.appleMusicPlaylist', 'Apple Music Playlist') : t('socialconnections.appleMusicPodcast', 'Apple Music Podcast'),
      url: appleMusicUrl,
      type: appleMusicType,
    };
    ...
    try {
      ...
      toast.success(`${appleMusicType === 'playlist' ? t('socialconnections.playlist', 'Playlist') : t('socialconnections.podcast', 'Podcast')} added!`);
      setAppleMusicUrl('');
  ...
  const handleRemovePlaylist = async (platform: 'spotify' | 'appleMusic', playlistId: string) => {
    ...
    try {
      ...
      toast.success(t("removed_from_profile", "Removed from profile"));
      onUpdate();
    } catch (error) {
      console.error('Error removing playlist:', error);
      toast.error(t("failed_to_remove", "Failed to remove"));
    }
  };
  ...
  const handleDisconnectMusic = async (platform: 'spotify' | 'appleMusic') => {
    ...
    try {
      ...
      toast.success(`${platform === 'spotify' ? t('socialconnections.spotify', 'Spotify') : t('socialconnections.appleMusic', 'Apple Music')} disconnected`);
      onUpdate();
    } catch (error) {
  ...
  return (
    <Card className="border-0 bg-card/30 backdrop-blur-md scroll-mt-8">
      <CardHeader>
        ...
        <CardDescription>{t('socialconnections.connectYourProfessionalProfilesAndShare', 'Connect your professional profiles and share your favorite music')}</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="p-4 bg-accent/5 border border-accent/20 rounded-lg">
          <p className="text-xs text-muted-foreground">
            <strong>{t("professional_presence", "Professional Presence:")}</strong> Connecting your profiles helps verify your identity and allows others to learn more about your professional background and interests.
          </p>
        </div>
        ...
        <div className="flex items-center justify-between p-4 border border-border rounded-lg bg-background/30 backdrop-blur-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium">{t("linkedin", "LinkedIn")}</p>
              <p className="text-sm text-muted-foreground">
                {socialConnections.linkedin ? t('socialconnections.connected', 'Connected') : t('socialconnections.notConnected', 'Not connected')}
              </p>
            </div>
          </div>
          {socialConnections.linkedin ? (
            ...
            <Button size="sm" onClick={() => onConnectSocial('linkedin_oidc')}>
              {t('socialconnections.connect', 'Connect')}
            </Button>
          )}
        </div>
        ...
        <div className="flex items-center justify-between p-4 border border-border rounded-lg bg-background/30 backdrop-blur-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium">{t("instagram", "Instagram")}</p>
              <p className="text-sm text-muted-foreground">
        ...
        <div className="flex items-center justify-between p-4 border border-border rounded-lg bg-background/30 backdrop-blur-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium">{t("x_twitter", "X (Twitter)")}</p>
              <p className="text-sm text-muted-foreground">
        ...
        <div className="flex items-center justify-between p-4 border border-border rounded-lg bg-background/30 backdrop-blur-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium">{t("github", "GitHub")}</p>
              <p className="text-sm text-muted-foreground">
        ...
        <div className="flex items-center justify-between p-4 border border-border rounded-lg bg-background/30 backdrop-blur-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium">{t("spotify", "Spotify")}</p>
              <p className="text-sm text-muted-foreground">
                {musicConnections.spotifyConnected ? t('socialconnections.connected', 'Connected') : t('socialconnections.notConnected', 'Not connected')}
              </p>
            </div>
          </div>
          {!musicConnections.spotifyConnected ? (
            <Button onClick={handleConnectSpotify} size="sm">{t('socialconnections.connect', 'Connect')}</Button>
          ) : (
        ...
        <div className="flex items-center justify-between p-4 border border-border rounded-lg bg-background/30 backdrop-blur-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium">{t("apple_music", "Apple Music")}</p>
              <p className="text-sm text-muted-foreground">
                {musicConnections.appleMusicConnected ? t('socialconnections.connected', 'Connected') : t('socialconnections.notConnected', 'Not connected')}
              </p>
            </div>
          </div>
          {!musicConnections.appleMusicConnected ? (
            <Button onClick={handleConnectAppleMusic} size="sm">{t('socialconnections.connect', 'Connect')}</Button>
          ) : (
            ...
            <div className="space-y-3 ml-13">
              ...
              {!showAppleMusicInput ? (
                ...
                <div className="space-y-3 p-3 border rounded-lg">
                  ...
                  <div className="space-y-2">
                    <Label>{t("apple_music_url", "Apple Music URL")}</Label>
                    <Input placeholder="https://music.apple.com/..." value={appleMusicUrl} onChange={(e) => setAppleMusicUrl(e.target.value)} />
                  </div>
                  <div className="flex gap-2">
                    <Button onClick={handleAddAppleMusicPlaylist} size="sm">{t("add", "Add")}</Button>
