    ...
    >
      <div className="relative flex-shrink-0">
        ...
        {!isGroup && otherParticipant && (
          <div className="absolute bottom-0 right-0">
            <OnlineStatusIndicator userId={otherParticipant.user_id} className="w-3 h-3 sm:w-3.5 sm:h-3.5 ring-2 ring-background" />
          </div>
        )}
      ...
      <div className="flex-1 min-w-0 overflow-hidden">
        <div className="flex items-start justify-between gap-1.5 sm:gap-2 mb-0.5 sm:mb-1">
          <h4 className="font-semibold text-xs sm:text-sm truncate flex-1 min-w-0">{displayName}</h4>
          {conversation.last_message_at && (
            <span className="text-[10px] sm:text-xs text-muted-foreground whitespace-nowrap flex-shrink-0 leading-tight">
              {formatDistanceToNow(new Date(conversation.last_message_at), { addSuffix: true }).replace('about ', '').replace(' ago', '')}
            </span>
          )}
        </div>
