    ...
    mutationFn: async (goalData: { kpi_name: string; personal_target: number; notes?: string }) => {
      ...
      const { error } = await supabase
        ...
        .insert({
          ...
          official_target: kpi?.target || null,
                  ...
                  >
                    ...
                    {availableForGoals.map(kpi => (
                      <option key={kpi.name} value={kpi.name}>
                        {kpi.name} (Current: {kpi.value?.toLocaleString()})
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  {formData.kpi_name && (
                    <p className="text-xs text-muted-foreground">
                      Official target: {availableKPIs.find(k => k.name === formData.kpi_name)?.target?.toLocaleString() || 'N/A'}
                    </p>
                  )}
                </div>
