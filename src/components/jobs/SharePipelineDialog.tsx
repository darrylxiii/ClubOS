...
export function SharePipelineDialog({
  ...
  const fetchShares = async () => {
    ...
    const { data, error } = await supabase
      .from('job_pipeline_shares' as any)
      .select('*')
      .eq('job_id', jobId)
      .order('created_at', { ascending: false });

    if (!error) setShares((data as unknown as ShareLink[]) || []);
    setLoading(false);
  };
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (passwordEnabled && password.trim()) {
        // Hash the password using a DB function
        const { data: hashData, error: hashError } = await supabase.rpc(
          'hash_pipeline_share_password' as any,
          { _password: password.trim() }
        );
        // If the hash function doesn't exist yet, store plain temporarily (migration will fix)
        payload.password_hash = hashData || null;
      }
      ...
      const link = `${window.location.origin}/pipeline/${(share as any).token}`;
      setNewLink(link);
