...
const REVENUE_SOURCES: Record<string, RevenueSource> = {
  actual: { type: "actual", label: "Actual Hired Salary", priority: 1, confidence: 100, color: "emerald" },
  override: { type: "override", label: "Manual Override", priority: 2, confidence: 95, color: "blue" },
  candidate_avg: { type: "candidate_avg", label: "Avg Candidate Salary", priority: 3, confidence: 75, color: "amber" },
  job_salary: { type: "job_salary", label: "Job Salary Range", priority: 4, confidence: 60, color: "orange" },
  company_default: { type: "company_default", label: "Company Default", priority: 5, confidence: 40, color: "red" },
};
...
export function RevenuePreCalculation({ deals, stages }: RevenuePreCalculationProps) {
  ...
  const calculations = useMemo(() => {
    ...
    const dealCalculations = deals.map((deal) => {
      ...
      else if (feeType === 'fixed') { fixedFeeCount++; }
      else if (feeType === 'hybrid') { hybridFeeCount++; }
      else { percentageFeeCount++; }
      ...
      if (effectiveConfidence >= 75) { highConfidenceValue += weightedValue; }
      else if (effectiveConfidence >= 50) { mediumConfidenceValue += weightedValue; }
      else { lowConfidenceValue += weightedValue; }

      return {
        dealId: deal.id, title: deal.title, company: deal.company_name,
        feeType, feePercentage, feeFixed, feeAmount, probability,
        weightedValue, source, confidence: effectiveConfidence, hasFee,
      };
    });

    const avgConfidence = dealCalculations.length > 0 
      ? dealCalculations.reduce((sum, d) => sum + d.confidence, 0) / dealCalculations.length : 0;

    return {
      totalPipeline: metrics?.total_pipeline || 0,
      weightedPipeline: metrics?.weighted_pipeline || 0,
      highConfidenceValue, mediumConfidenceValue, lowConfidenceValue,
      avgConfidence, missingFeeCount, missingSalaryCount,
      fixedFeeCount, percentageFeeCount, hybridFeeCount,
      dealCalculations, dealCount: deals.length,
    };
  }, [deals, metrics]);
  ...
  return (
    <div className="space-y-6">
      {/* Confidence + Deal Count (unique to this view) */}
            ...
            <CardContent className="pt-6">
              ...
              <p className="text-xs text-muted-foreground">
                {calculations.missingFeeCount > 0 
                  ? `${calculations.missingFeeCount} missing fee config`
                  : 'All deals have fee data'}
              </p>
            </CardContent>
          </Card>
        </motion.div>
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Progress value={calculations.weightedPipeline > 0 ? (calculations.highConfidenceValue / calculations.weightedPipeline) * 100 : 0} className="h-2 bg-muted" />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Progress value={calculations.weightedPipeline > 0 ? (calculations.mediumConfidenceValue / calculations.weightedPipeline) * 100 : 0} className="h-2 bg-muted" />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Progress value={calculations.weightedPipeline > 0 ? (calculations.lowConfidenceValue / calculations.weightedPipeline) * 100 : 0} className="h-2 bg-muted" />
            </div>
          </div>
